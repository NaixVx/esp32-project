...
esp_err_t ConfigManager::saveToNVS() {
  ...
  persisted_.header.struct_size =
      static_cast<uint16_t>(sizeof(PersistedConfig));
...
esp_err_t ConfigManager::loadFromNVS() {
  ...
  if (header->version != 1) {
    ESP_LOGW(TAG, "Unknown config version: %u",
  ...
  const size_t copy_len =
      std::min(static_cast<size_t>(header->struct_size),
...
void ConfigManager::setDefaults() {
  ...
  std::strncpy(persisted_.info.device_name, "esp32-demo",
               DEVICE_NAME_MAX_LEN - 1);
  std::strncpy(persisted_.info.device_type, "generic", DEVICE_TYPE_MAX_LEN - 1);
  std::strncpy(persisted_.info.firmware_version, "0.0.1",
...
bool ConfigManager::isValid() {
  ...
  auto within = [](const char *s, size_t n) { return strnlen(s, n) < n; };
