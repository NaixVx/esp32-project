...
void Handlers::registerDeviceEndpoints(httpd_handle_t server, void *ctx) {
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG, "Failed to register GET /api/device/info: %s",
             esp_err_to_name(err));
  }
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG, "Failed to register POST /api/device/info: %s",
             esp_err_to_name(err));
  }
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG, "Failed to register OPTIONS /api/device/info: %s",
             esp_err_to_name(err));
  }
}
