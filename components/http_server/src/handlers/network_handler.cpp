...
void Handlers::registerNetworkEndpoints(httpd_handle_t server, void *ctx) {
  ...
  if (err == ESP_OK)
    ...
    ESP_LOGE(TAG, "Failed to register GET /api/network/status: %s",
             esp_err_to_name(err));
  ...
  if (err == ESP_OK)
    ...
    ESP_LOGE(TAG, "Failed to register POST /api/network/ap/set: %s",
             esp_err_to_name(err));
  ...
  if (err == ESP_OK)
    ...
    ESP_LOGE(TAG, "Failed to register POST /api/network/sta/set: %s",
             esp_err_to_name(err));
}
